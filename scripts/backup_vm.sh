...
if ls /mnt/pve/backups/dump/*_manual.vma.zst 1> /dev/null 2>&1; then
  if whip_alert_yesno "JA" "NEIN" "${whip_title}" "Alle vorhandenen, manuell erstellte VM Sicherungen in Deinem Backupverzeichnis werden gelöscht. Wenn Du sie behalten möchtest, sicher sie, bevor Du fortfährst. Dies gilt nicht für Sicherungen, die automatisch von Proxmox erstellt wurden.\n\nFortfahren?"; then
